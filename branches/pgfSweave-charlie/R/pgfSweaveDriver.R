...
pgfSweaveRuncode <- function(object, chunk, options) {
				...
				# Not sure why the hell this is here. Looks like it
				# causes the evaluator to bail if the calling program
				# is a non-standard implementation of the S language
				# i.e. not S++ or R.
        if(!(options$engine %in% c("R", "S"))){
                return(object)
        }

				# If Sweave is not being quiet- i.e. console output
				# is desired, here is where we babble about  the options
				# invoked on this code chunk.
        ...
        if(!object$quiet){
				...
				# This function uses information in options to
				# generate a string of the form "filename-chunkname"
        chunkprefix <- RweaveChunkPrefix(options)

				# No idea what the split option invokes.
        if(options$split){
				...
				# this is a connection to the output file.
        else    chunkout <- object$output

				# Sweave is about to overwrite system defaults- save a copy
				# so they can be restored afterwards. (Should this be done
				# and redone for every code chunk?) Why not do it in the main
				# Sweave function?
        saveopts <- options(keep.source=options$keep.source)
        on.exit(options(saveopts))

				# Interestingly, SweaveHooks() dosn't appear to change any
				# global options, it just executes any function names that
				# happen to be in getOption("SweaveHooks"). The whole function
				# is pretty screwy.
        SweaveHooks(options, run=TRUE)
    
        # Run the chunk text through the R parser. This splits the
				# text into a vector of commands and checks each command for
				# errors.
        chunkexps <- try(parse(text=chunk), silent=TRUE)
				# If parse found errors, RweaveTryStop prints a warning message
				# and causes the whole operation to bail.
